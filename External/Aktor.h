...
int netInAktor(int PowerGrid, int AktorPower, int AktorMinOn, int AktorMinOff, int AktorPin, int AktorPosition)
{
...
int solarAktor(int PowerPVI, int AktorPowerOn, int AktorPowerOff, int AktorMinOn, int AktorMinOff, int AktorPin, int AktorPosition)
{
  int AktorState = digitalRead(AktorPin);
  if (AktorState == Deaktiv){
    int counterMinOff = readAktor(AktorPosition + MinOff);
...
int batteryAktor(int BatterySOC, int AktorPercentOn, int AktorPercentOff, int AktorMinOn, int AktorMinOff, int AktorPin, int AktorPosition)
{
  int AktorState = digitalRead(AktorPin);
  if (AktorState == Deaktiv){
    int counterMinOff = readAktor(AktorPosition + MinOff);
              ...
              int AktorTyp, int AktorPin, int AktorPosition)
{
  if (AktorTyp > 0){
    if (AktorTyp == 1)
      netInAktor(PowerGrid, AktorPowerOn, AktorPin, AktorMinOn, AktorMinOff, AktorPosition);
    else if (AktorTyp == 2)
      solarAktor(PowerPVI, AktorPowerOn, AktorPowerOff, AktorMinOn, AktorMinOff, AktorPin, AktorPosition);
    else if (AktorTyp == 3)
      batteryAktor(BatterySOC, AktorPercentOn, AktorPercentOff, AktorMinOn, AktorMinOff, AktorPin, AktorPosition);
  }
  return 1;
}
...
int checkAktor()
{
  if (useAktor == 1){
    ...
    int PowerPVI = readRscp(PosPVI);
    int BatterySOC = readRscp(PosSOC);
    Aktor(PowerGrid, PowerPVI, BatterySOC, Aktor1PowerOn, Aktor1PowerOff, Aktor1PercentOn, Aktor1PercentOff, Aktor1MinOn, Aktor1MinOff, Aktor1Typ, Aktor1Pin, Aktor1Position);
    Aktor(PowerGrid, PowerPVI, BatterySOC, Aktor2PowerOn, Aktor2PowerOff, Aktor2PercentOn, Aktor2PercentOff, Aktor2MinOn, Aktor2MinOff, Aktor2Typ, Aktor2Pin, Aktor2Position);
    Aktor(PowerGrid, PowerPVI, BatterySOC, Aktor3PowerOn, Aktor3PowerOff, Aktor3PercentOn, Aktor3PercentOff, Aktor3MinOn, Aktor3MinOff, Aktor3Typ, Aktor3Pin, Aktor3Position);
    Aktor(PowerGrid, PowerPVI, BatterySOC, Aktor4PowerOn, Aktor4PowerOff, Aktor4PercentOn, Aktor4PercentOff, Aktor4MinOn, Aktor4MinOff, Aktor4Typ, Aktor4Pin, Aktor4Position);
    Aktor(PowerGrid, PowerPVI, BatterySOC, Aktor5PowerOn, Aktor5PowerOff, Aktor5PercentOn, Aktor5PercentOff, Aktor5MinOn, Aktor5MinOff, Aktor5Typ, Aktor5Pin, Aktor5Position);
    return 1;
  }
  return 0;
}
